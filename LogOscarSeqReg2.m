...
function [CoefMatrix dfMatrix NegLogLikelihoodMatrix] = OscarSeqReg(X, y, cvalues, propvalues, initcoef)
...
NegLogLikelihoodMatrix = zeros(1,length(propvalues),length(cvalues));
...
for ccount = 1:length(cvalues)
    ...
    for propcount = 1:length(propvalues)
        ...
        [coefs, df, neg_log_likelihood, conv] = OscarSeqOpt(tbound, cvalue, Xmatrix, y, start, OrderMatrix);
        CoefMatrix(:,propcount,ccount) = coefs;
        NegLogLikelihoodMatrix(:,propcount,ccount) = neg_log_likelihood;
        dfMatrix(:,propcount,ccount) = df;
        if conv == 0
            fprintf('Optimization may has not converged properly for c = %g and prop = %g.\\n', cvalue, propvalues(propcount));
        else
